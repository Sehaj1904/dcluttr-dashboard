...
import type { SemiPieChartData } from "@/lib/types"
...
interface SemiPieChartCardProps {
  title: string
  data: SemiPieChartData
}

export default function SemiPieChartCard({ title, data }: SemiPieChartCardProps) {
      ...
      if (ctx) {
        chartInstance.current = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [
              {
                data: data.cities.map((city) => city.percentage),
                backgroundColor: data.cities.map((city) => city.color),
                borderWidth: 0,
                circumference: 180,
                rotation: 270,
              },
      ...
      <div className="p-4">
        <div className="h-48 relative">
          ...
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center">
            ...
            <div className="text-2xl font-semibold text-[#161616]">{data.totalValue}</div>
            <div
              className={`flex items-center justify-center text-sm ${
                data.totalPercentChange >= 0 ? "text-[#42BE65]" : "text-[#FF6B6B]"
              }`}
            >
              {data.totalPercentChange >= 0 ? (
                <ArrowUp size={14} className="mr-1" />
              ) : (
                <ArrowDown size={14} className="mr-1" />
              )}
              <span>{Math.abs(data.totalPercentChange)}%</span>
            </div>
          </div>
        </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div
                  className={`flex items-center w-16 text-sm ${
                    city.percentChange >= 0 ? "text-[#42BE65]" : "text-[#FF6B6B]"
                  }`}
                >
